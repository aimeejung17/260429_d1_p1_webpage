# NoCodeDXConsulting (Single Page)

`NoCodeDXConsulting` 소개용 **싱글 페이지 랜딩**입니다. (바닐라 `HTML/CSS/JS`만 사용)
...
- 화면 하단 **고객 문의 폼**
  ...
  - 폼 필드: 현재 상황(한 줄) / 이름 / 이메일 / 문의사항(메시지)
- 스크롤 이동(`data-scroll`), 섹션 reveal 애니메이션, 모바일 메뉴 토글

## 파일 구조

- `index.html`: 페이지 마크업(섹션/폼/토글 UI 포함)
- `style.css`: Claude 계열 다크 무드 + 라이트 테마 스타일(테마 변수 기반)
- `script.js`: 토글/문의 `mailto:` 구성 로직
